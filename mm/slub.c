...
{
	return s->node[node];
	/*! 20140426 kmem_cache->node[node] 값을 리턴 */
}
...
/*! 20140412 oo 방식의 상위 16bit(buddy에 쓰이는 order)를 리턴한다. */
static inline int oo_order(struct kmem_cache_order_objects x)
{
	return x.x >> OO_SHIFT;
}

/*! 20140412 oo 방식의 하위 16bit(object 갯수)를 리턴한다. */
...
static __always_inline void slab_lock(struct page *page)
{
	...
	/*! 20140517 page에 대한 PG_locked bit락을 획득한다. */
}
		...
		const char *n)
{
...
#endif
	{
		 ...
		 */
			...
			return 1;
			/*! 20140531 
			 * page->freelist를 freelist_new로, page->counters를 counters_new로
			 * atomic하게 변경시키는 것이 성공하면 1 리턴
			 */
		}
		slab_unlock(page);
	}
	...
	return 0;
	/*! 20140531 atomic하게 수행되지 않으면 0이 리턴된다.
	 * page->freelist를 다른 곳에서 변경한 것이다.
	 */
}
...
static inline void inc_slabs_node(struct kmem_cache *s, int node, int objects)
{
	...
	if (likely(n)) {
		...
		/*! 20140426 n->nr_slabs++, n->total_objects += objects */
	}
}
...
/*! 20140531 CONFIG_SLUB_DEBUG가 꺼져있다고 가정하면 여기 실행됨 */
...
static inline void inc_slabs_node(struct kmem_cache *s, int node,
							int objects) {}
/*! 20140426 CONFIG_SLUB_DEBUG가 꺼져있다고 가정하면 여기 실행됨 */
...
static struct page *allocate_slab(struct kmem_cache *s, gfp_t flags, int node)
{
	...
	/*! 20140426 oo(상위 16bit)에 설정되어 있는 order 크기의 page를 buddy에서 할당받음 */
	if (unlikely(!page)) {
		oo = s->min;
		/*
		...
		/*! 20140426 page 할당에 실패했을 경우, flags와 order를 변경하여 다시 할당 */
	...
	/*! 20140426 kmemcheck_enabled가 0이므로 실행안됨 */
	...
	/*! 20140426 page->objects: slab으로 할당할 object의 갯수 */
	mod_zone_page_state(page_zone(page),
		(s->flags & SLAB_RECLAIM_ACCOUNT) ?
		NR_SLAB_RECLAIMABLE : NR_SLAB_UNRECLAIMABLE,
		1 << oo_order(oo));
	/*! 20140426
	 * page가 속한 zone의 vm_stat[NR_SLAB_RECLAIMABLE or NR_SLAB_UNRECLAIMABLE]에
	 * (1 << oo_order(oo))(buddy에 쓰이는 order)를 더한다.
	 */
				...
				void *object)
{
	...
	/*! 20140426 CONFIG_SLUB_DEBUG: y이지만 debug이므로 pass */
	if (unlikely(s->ctor))
		s->ctor(object);
		/*! 20140426 아직 설정한 적 없으므로 NULL */
}
...
static struct page *new_slab(struct kmem_cache *s, gfp_t flags, int node)
{
	...
	/*! 20140426 slab으로 사용할 page 할당받음 */
	if (!page)
		goto out;

	order = compound_order(page);
	/*! 20140426 PG_compound가 설정 안되어 있으므로 order: 0 */
	inc_slabs_node(s, page_to_nid(page), page->objects);
	/*! 20140426 s->node의 정보를 update한다. */
	memcg_bind_pages(s, order);
	/*! 20140426 아무것도 안함 */
	page->slab_cache = s;
	/*! 20140426 할당받은 page의 slab_cache를 현재 kmem_cache로 설정한다. */
	__SetPageSlab(page);
	/*! 20140426 page->flags에 PG_slab을 설정한다. 현재 struct page가 slab으로 사용됨을 표시 */
	if (page->pfmemalloc)
		/*! 20140426 ALLOC_NO_WATERMARKS 가 설정안되어 있으므로 실행안됨.
		 * get_page_from_freelist 함수에서 확인
		 */
		SetPageSlabPfmemalloc(page);

	start = page_address(page);
	/*! 20140426 page의 va를 가져온다. */

	if (unlikely(s->flags & SLAB_POISON))
		memset(start, POISON_INUSE, PAGE_SIZE << order);
		/*! 20140426 현재 할당받은 page 크기만큼 POISON_INUSE로 채움. */

	last = start;
	for_each_object(p, s, start, page->objects) {
	/*! 20140426 page->objects: 102, (s)->size: 40이라 가정한다. 
	 * for (p = (start); p < (start) + (page->objects) * (s)->size; p += (s)->size)
	 */
		setup_object(s, page, last);
		/*! 20140426 s->ctor 함수가 있으면 실행. 없으면 아무동작 안함 */
		set_freepointer(s, last, p);
		/*! 20140426 각 slab objects의 첫 4byte를 해당 slab의 시작주소(va)로 초기화 */
		last = p;
	}
	setup_object(s, page, last);
	set_freepointer(s, last, NULL);
	/*! 20140426 마지막 slab을 NULL로 셋팅 */

	page->freelist = start;
	page->inuse = page->objects;
	page->frozen = 1;
	/*! 20140426
	 * page->freelist에 초기화한 page의 시작주소를 할당
	 * page->inuse: page->frozen이 1이므로, 사용가능한 slab의 갯수
	 */
out:
	return page;
}
...
static void __free_slab(struct kmem_cache *s, struct page *page)
{
	...
	/*! 20140531 CONFIG_KMEMCHECK 가 없으므로 아무일도 안함 */

	mod_zone_page_state(page_zone(page),
		(s->flags & SLAB_RECLAIM_ACCOUNT) ?
		NR_SLAB_RECLAIMABLE : NR_SLAB_UNRECLAIMABLE,
		-pages);
	/*! 20140531 해당 slab type에 따라 page가 속한 갯수를 감소시킨다. */

	__ClearPageSlabPfmemalloc(page);
	/*! 20140531 &page->flags 의 PG_active 자리의 bit를 clear한다. */
	__ClearPageSlab(page);
	/*! 20140531 &page->flags 의 PG_slab 자리의 bit를 clear한다. */
...
static void free_slab(struct kmem_cache *s, struct page *page)
{
	if (unlikely(s->flags & SLAB_DESTROY_BY_RCU)) {
		...
		__free_slab(s, page);
	/*! 20140531 slab으로 사용하던 page를 buddy 시스템에 반환 */
}
		...
		int mode, int *objects)
{
	...
	*objects = new.objects - new.inuse;
	...
	new.frozen = 1;
	/*! 20140517 object = NULL이고, 아래와 같이 호출된 경우,
	 * new변수(struct page)는 아래와 같이 설정됨   
...
static void init_kmem_cache_cpus(struct kmem_cache *s)
{
	...
	for_each_possible_cpu(cpu)
		...
		/*! 20140517 각 cpu별로 cpu_slab->tid를 cpu 번호로 설정한다. */
}
...
static void deactivate_slab(struct kmem_cache *s, struct page *page, void *freelist)
{
	...
	if (page->freelist) {
	/*! 20140531 page->freelist 가 비어있지 않으면 tail을 DEACTIVATE_TO_TAIL로 바꾼다. */
		stat(s, DEACTIVATE_REMOTE_FREES);
		/*! 20140531 아무일도 안함 */
		tail = DEACTIVATE_TO_TAIL;
		/*! 20140531 DEACTIVATE_TO_TAIL: Cpu slab was moved to the tail of partials */
	}
	...
	/*! 20140531 cpu_slab에 freelist가 있고, 해당 freelist의 다음 object가 비어있는 경우 실행 */
		void *prior;
		unsigned long counters;
		...
		/*! 20140531 page의 list의 방향을 반대로 setting한다. */
		/*! 20140531 cpu slab의 freelist를 없애고 page slab의 freelist로 바꾸기 위한 것으로 추정 */
...
redo:
	...
	/*! 20140531 new.freelist를 old.freelist(page->freelist) 로 설정 */
	...
	if (!__cmpxchg_double_slab(s, page,
				...
				new.freelist, new.counters,
				"unfreezing slab"))
		/*! 20140531 page->freelist = new.freelist, page->counters = new.counters */
		goto redo;
		...
		struct kmem_cache_cpu *c)
{
	...
	/*! 20140531 cpu_slab에 partial page가 남아 있으면 계속 수행 */
		struct page new;
		struct page old;

		c->partial = page->next;
		/*! 20140531 가져온 다음 partial 가리킴 */

		n2 = get_node(s, page_to_nid(page));
		/*! 20140531 우리는 node가 하나이므로 n 과 n2는 항상 동일함 */
		if (n != n2) {
			if (n)
				spin_unlock(&n->list_lock);
...
static inline void flush_slab(struct kmem_cache *s, struct kmem_cache_cpu *c)
{
	...
	c->page = NULL;
	c->freelist = NULL;
	/*! 20140531 현재cpu slab에서 제거하고 c->tid 는 4 증가 */
}
...
static inline bool pfmemalloc_match(struct page *page, gfp_t gfpflags)
{
	if (unlikely(PageSlabPfmemalloc(page)))
		/*! 20140517 page에 PG_Active flag가 설정되면 여기가 실행된다 */
		return gfp_pfmemalloc_allowed(gfpflags);

	/*! 20140517 현재 PG_Active flag설정되지 않으므로 true리턴됨 */
	return true;
}
			  ...
			  unsigned long addr, struct kmem_cache_cpu *c)
{
	...
	/*! 20140517 현재 cpu의 cpu_slab pointer를 얻어온다. */
#endif
		...
		gfp_t gfpflags, int node, unsigned long addr)
{
	...
	/*! 20140517 hook은 실행하지 않는다.  */
	if (slab_pre_alloc_hook(s, gfpflags))
		return NULL;

	/*! 20140517 cgroup은 실행하지 않는다.  */
...
redo:
	...
	preempt_disable();
	c = __this_cpu_ptr(s->cpu_slab);
	/*! 20140517 현재 cpu의 percpu cpu_slab 변수 포인터를 얻는다 */
...
{
	n->nr_partial = 0;
	/*! 20140426 n->nr_partial: 실제 사용하는 slab 갯수 */
...
static inline int alloc_kmem_cache_cpus(struct kmem_cache *s)
{
	...
	/*! 20140517 kmem_cache_cpu 자료구조인 cpu_slab을 위해 percpu 공간을 할당한다. */
	if (!s->cpu_slab)
		return 0;
	/*! 20140517 할당받지 못했으면 리턴 0 */

	init_kmem_cache_cpus(s);
	/*! 20140517 kmem_cache_cpu의 cpu 멤버를 cpu 번호로 초기화 */
...
static void early_kmem_cache_node_alloc(int node)
{
	...
	/*! 20140426 buddy에서 page를 할당받아 새로운 slab으로 초기화한다. */

	BUG_ON(!page);
	if (page_to_nid(page) != node) {
		/*! 20140426 NUMA가 아니므로 여기는 실행되지 않음 */
	...
	n = page->freelist;
	/*! 20140426 첫번째 slab object를 kmem_cache_node로 쓰겠다는 것 */
	BUG_ON(!n);
	page->freelist = get_freepointer(kmem_cache_node, n);
	/*! 20140426 page->freelist의 변수를 두번째 slab object의 시작위치를 가리키게 한다. */
	page->inuse = 1;
	page->frozen = 0;
	/*! 20140426 page->frozen이 0으므로, 실제 사용하는 slab의 갯수 */
	...
	/*! 20140426 n의 nr_partial, list_lock, partial를 초기화 한다. */
	inc_slabs_node(kmem_cache_node, node, page->objects);
	/*! 20140426 CONFIG_SLUB_DEBUG가 꺼져있으면 아무것도 안함 */

	add_partial(n, page, DEACTIVATE_TO_HEAD);
	/*! 20140426 n의 list에 page 추가 */
}
...
static int init_kmem_cache_nodes(struct kmem_cache *s)
{
	...
	for_each_node_state(node, N_NORMAL_MEMORY) {
		...
		if (slab_state == DOWN) {
			...
			/*! 20140426 early slab을 초기화: slab을 관리하기 위한 kmem_cache_node 초기화 */
			/*! 20140426 여기까지 스터디 */
			continue;
		}
...
static int kmem_cache_open(struct kmem_cache *s, unsigned long flags)
{
	...
	/*! 20140517 pcpu kmem_cache_cpu 자료구조를 위한 공간 할당 및 초기화 */
		return 0;
...
static struct kmem_cache * __init bootstrap(struct kmem_cache *static_cache)
{
	 ...
	 */
	__flush_cpu_slab(s, smp_processor_id());
	/*! 20140531 cpu_slab page 및 partial 제거 */
	for_each_node_state(node, N_NORMAL_MEMORY) {
	/*! 20140531 for ( (node) = 0; (node) == 0; (node) = 1) */
		struct kmem_cache_node *n = get_node(s, node);
		struct page *p;
		...
		/*! 20140531 partial page에 연결된 모든 page의 slab_cache가 s(새로만든 kmem_cache)를 가리키도록 한다. */
	}
	list_add(&s->list, &slab_caches);
	/*! 20140531 전역 slab_caches에 s(새로만든 kmem_cache)->list를 연결한다. */
	return s;
}
...
void __init kmem_cache_init(void)
{
	...
	slab_state = PARTIAL;
	/*! 20140517 드디어 slub 상태 partial. kmem_cache_node 사용가능 */

	create_boot_cache(kmem_cache, "kmem_cache",
			offsetof(struct kmem_cache, node) +
				nr_node_ids * sizeof(struct kmem_cache_node *),
		       SLAB_HWCACHE_ALIGN);
	/*! 20140517 kmem_cache slab을 위한 자료구조 초기화.
	 * node는 갯수만큼 할당한다.
	 */

	/*! 20140524 여기까지 진행함. bootstrap부터 자세히 다시 봐야함 */
	kmem_cache = bootstrap(&boot_kmem_cache);
	/*! 20140531 boot_kmem_cache를 kmem_cache로 전역화시킨다. */
	...
	/*! 20140531 boot_kmem_cache_node를 kmem_cache_node로 전역화시킨다. */
...
int __kmem_cache_create(struct kmem_cache *s, unsigned long flags)
{
	...
	/*! 20140517 (percpu) kmem_cache를 위한 자료구조 할당 및 초기화 */
	if (err)
		return err;
