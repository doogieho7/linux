...
{
	/*! 20131026 vm_struct에는 prev가 없기 때문에 2중 포인터를 이용해 접근한다. */

	struct vm_struct *tmp, **p;

	/*! 20131026 본 vmalloc 초기화 후에 early 함수를 사용하면 panic */
	...
	for (p = &vmlist; (tmp = *p) != NULL; p = &tmp->next) {
		...
		if (tmp->addr >= vm->addr) {
			...
			/*! 20131026 탐색중인 list와 영역이 겹치지 않으면 완료 */
			break;
		} else
			BUG_ON(tmp->addr + tmp->size > vm->addr);
	}
	/*! 20131026 올바른 위치에 삽입한다. *p는 prev_list->next */
	vm->next = *p;
	*p = vm;
	/*! 20131026 p -> n 에서 new(vm)을 삽입하면 p -> new -> n 가 된다. */
}
